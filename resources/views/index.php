...
<head>
    ...
    <link rel="stylesheet" href="<?= asset('vendor/translations-manager/bootstrap.min.css') ?>" rel="stylesheet">
    <link href="<?= asset('vendor/translations-manager/bootstrap-editable.css') ?>" rel="stylesheet"/>
    <script src="<?= asset('vendor/translations-manager/jquery-1.11.0.min.js') ?>"></script>
    <script src="<?= asset('vendor/translations-manager/bootstrap.min.js') ?>"></script>
    <script src="<?= asset('vendor/translations-manager/bootstrap-editable.min.js') ?>"></script>
    <script src="<?= asset('vendor/translations-manager/jquery.cookie.js') ?>"></script>
...
<header class="navbar navbar-static-top" id="top" role="banner" style="border-bottom:2px solid #ccc">
    <div class="container-fluid">
        <div class="navbar-header">
            ...
            <a href="<?php echo action('\Barryvdh\TranslationManager\Controller@getIndex') ?>" class="navbar-brand">
                <img src="<?= asset('/images/logo/' . config('app.logo_saas_admin')) ?>" style="max-height:25px;display:inline">
                Translation Manager
            </a>
        </div>
    </div>
...
<div class="container-fluid">
    <button onclick="document.location.href='/dashboard'" class="btn btn-success">&laquo; Back to Cannavigia</button><br/><br/>

    <p>Warning, translations are not visible until they are exported back to the app/lang files. When done, ask Pelle to publish the translations.</p>
    <div class="alert alert-success success-import" style="display:none;">
        <p>Done importing, processed <strong class="counter">N</strong> items! Automatically reloading this page to refresh the groups!</p>
    </div>
    ...
    <p>
        ...
        <div class="checkbox">
            <label>
                <input value="1" type="checkbox" id="toggle_display_missing_only" checked> <b>Display missing translations only</b>
            </label>
        </div>
            ...
            <div class="form-group">
                <select name="group" id="group" class="form-control group-select">
                    <?php foreach($groups as $key => $value): ?>
                        <?php if($key != '_json' && $key != 'vendor/authentication-log'): ?>
        ...
        <?php else: ?>
            ...
            <table class="table">
                ...
                <tbody>
                    ...
                    <?php foreach($allTranslationsByGroup as $group => $groupTranslations): ?>
                        <?php if($groupTranslations && isset($groupTranslations['translations']) && $group != '_json' && $group != 'vendor/authentication-log'): ?>
                            <tr>
                                ...
                                <?php foreach ($groupTranslations['translations'] as $key => $translation): ?>
                                    <tr
                                        data-has-empty=""
                                        ...
                                        <?php foreach ($locales as $locale): ?>
                                            ...
                                            <td>
                                                <a href="#edit"
                                                   ...
                                                   data-type="textarea"
                                                   data-pk="<?php echo $t ? $t->id : 0 ?>"
                                                   data-url="<?php echo $groupTranslations['editUrl'] ?>"
                                                   data-title="Enter translation"><?php echo $t ? htmlentities($t->value, ENT_QUOTES, 'UTF-8', false) : '' ?></a>
                                            </td>
                                        <?php endforeach; ?>
                                    </tr>
...
<script>
    function onlyEmpty() {
        $.each($('tr:has(a.editable)'), function() {
    ...
    $(function() {
        ...
        $('#toggle_display_missing_only').click(function() {
